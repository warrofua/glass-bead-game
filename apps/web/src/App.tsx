...
export default function App() {
  ...
  const castBead = async () => {
    if (!playerId || !state || !isMyTurn) return;
  ...
  const bindSelected = async () => {
    if (!playerId || !state || selected.length !== 2 || !isMyTurn) return;
  ...
  const suggestBead = async () => {
    if (!state || !playerId || !isMyTurn) return;
    try {
  ...
  return (
    <div className="min-h-screen p-4 grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4">
      <aside className="bg-[var(--panel)] rounded-2xl p-4 space-y-3 shadow">
        <h1 className="text-xl font-semibold">Glass Bead Game — Ritual</h1>
        ...
        <div className="pt-4 space-y-2">
          ...
          <div className="flex gap-2">
            <button
              onClick={suggestBead}
              disabled={!isMyTurn}
              className="w-1/2 px-3 py-2 bg-zinc-800 rounded hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Suggest with AI
            </button>
            <button
        ...
        <p className="text-xs text-[var(--muted)] pt-4">Moves: cast, bind. Focus on resonance and clarity.</p>
      </aside>
                        ...
                        {scroll.strongPaths.map((p, idx) => (
                          <li key={idx}>
                            <button
                              ...
                              className={`text-xs underline ${selectedPath === idx ? 'font-bold' : ''}`}
                            >
