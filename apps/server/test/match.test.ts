...
test('match handles cast/bind flow and returns AI judgment', async (t) => {
  ...
  const bead2 = { id: `b_${Math.random().toString(36).slice(2,8)}`, ownerId: p2.id, modality: 'text', content: 'b', complexity: 1, createdAt: Date.now() };
  await postMove({ id: `m_${Math.random().toString(36).slice(2,8)}`, playerId: p2.id, type: 'cast', payload: { bead: bead2 }, timestamp: Date.now(), durationMs: 0, valid: true });
  ...
  assert.equal(Object.keys(state.edges).length, 1);
  ...
  assert.ok(scroll.winner === p1.id || scroll.winner === p2.id || scroll.winner === undefined);
